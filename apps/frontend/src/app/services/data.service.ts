...
export class DataService {
  ...
  listUnread(groupId) {
    return this.list = this.angularDb.list('/accounts/' + this.authService.getUser()?.uid + '/groups/' + groupId + '/messagesRead/')
  }
  ...
  listUnreadStatus(userId) {
    return this.angularDb.list('/messages/' + this.authService.getUser()?.uid + '/' + userId + '/').query.orderByChild('read' && 'userId').equalTo('unread' && userId)
  }

  listUnreadChat(userId, key) {
    return this.list = this.angularDb.list('/messages/' + this.authService.getUser()?.uid + '/' + userId + '/' + key);
  }
  ...
  readSender(currerntUser) {
    return this.user = this.angularDb.object("/messages/" + this.authService.getUser()?.uid + '/' + currerntUser)
  }
  userBocks(userId) {
  ...
  postuserBock(userId) {
    return this.user = this.angularDb.object("/accounts/" + this.authService.getUser()?.uid + "/blocks/")
  }
  postsenderBock(userId) {
    return this.user = this.angularDb.object("/accounts/" + userId + "/blocks/")
  }
  userblocksBy(userId) {
    return this.user = this.angularDb.object("/accounts/" + this.authService.getUser()?.uid + "/userblocks/")
  }
  //get all users
  getUsers() {
