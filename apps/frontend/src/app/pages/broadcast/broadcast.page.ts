...
export class BroadcastPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataServices.groups(this.broadcastId).valueChanges().subscribe((broadcast) => {
      ...
      if (broadcast.admin) {
        let index = _.indexOf(broadcast.admin, this.authService.getUser()?.uid);
        if (index > - 1) {
          this.isAdmin = true;
        }
      }
    ...
    if (this.userId) {
      var promise = new Promise((resolve, reject) => {
        ...
        for (let i = 0; i < this.broadcast.members.length; i++) {
          this.afDB.database.ref('/messages/').child(this.broadcast.members[i]).child(this.authService.getUser()?.uid).push(messages).then((snap) => {
            var keys = snap.key;
            snap.update({
              key: keys
            })
          })
          this.afDB.database.ref('conversations').child(this.broadcast.members[i]).orderByChild('userId').equalTo(this.authService.getUser()?.uid).once('value', snapshot => {
