...
export class StatusPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getStoryCurrent(this.authService.getUser()?.uid).once("value", snap => {
      let Data = snap.val();
      if (Data) {
      ...
      storylist.forEach((post) => {
        ...
        this.dataService.getview(tempData.key).valueChanges().subscribe((likes) => {
          ...
          let isView = _.findKey(likes, view => {
            return view == this.authService.getUser()?.uid;
          })
          // let do some logic
          if (isView) {
  ...
  send(url) {
    var promise = new Promise((resolve, reject) => {
      this.angularDb.database.ref('/story').push({
        ...
        postBy: this.authService.getUser()?.uid,
        image: url
      }).then((success) => {
        resolve(true);
