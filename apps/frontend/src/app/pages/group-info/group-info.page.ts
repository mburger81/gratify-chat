...
export class GroupInfoPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.groups(this.groupId).valueChanges().subscribe((group) => {
      this.group = group;
      if (group.admin) {
        let index = _.indexOf(group.admin, this.authService.getUser()?.uid);
        if (index > - 1) {
          this.isAdmin = true;
        }
      }
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (data) => {
            // Delete group image.
            this.angularDb.object('/accounts/' + this.authService.getUser()?.uid + '/groups/' + this.groupId).remove().then(() => {
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.dataService.groups(this.groupId).update({
              ...
              firebase.database().ref('groups').child(this.groupId).child('messages').push({
                date: new Date().toString(),
                sender: this.authService.getUser()?.uid,
                type: 'system',
  ...
  async deleteConversation() {
    await this.angularDb.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('key').equalTo(this.groupId).once('value', snap => {
      var res = snap.val();
      if (res != null) {
        let store = Object.keys(res)
        this.angularDb.database.ref('conversations').child(this.authService.getUser()?.uid).child(store[0]).remove();
      }
    })
  }
