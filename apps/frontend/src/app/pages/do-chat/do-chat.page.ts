...
export class DoChatPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once("value", snap => {
    ...
    this.dataService.getUser(this.userId).valueChanges().subscribe((user) => {
      ...
      this.dataService.userBocks(this.authService.getUser()?.uid).valueChanges().subscribe((blocks) => {
  ...
  readMessage() {
    ...
    this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once("value", snap => {
      ...
      this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).child(key[0]).once("value", value => {
        let me = value.val();
        if (me.me == 'you') {
          this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once('value', snapshot => {
            var res = snapshot.val();
            if (res != null) {
              let key = Object.keys(res)
              // if the user is unread message make to read Message,,
              this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).child(key[0]).update({ read: 'read' })
            }
          }).then(() => {
            // then pass ot the current user
            this.afDB.database.ref('conversations').child(this.userId).orderByChild('userId').equalTo(this.authService.getUser()?.uid).once('value', snapshot => {
              var res = snapshot.val();
              if (res != null) {
                let key = Object.keys(res)
          ...
          this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once('value', snapshot => {
            ...
            if (res != null) {
              let key = Object.keys(res)
              this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).child(key[0]).update(updateRead)
            }
          }).then(() => {
            this.afDB.database.ref('conversations').child(this.userId).orderByChild('userId').equalTo(this.authService.getUser()?.uid).once('value', snapshot => {
              var res = snapshot.val();
  ...
  readMessageSender() {
    ...
    this.afDB.database.ref('messages').child(this.userId).child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once('value', snapshot => {
      var res = snapshot.val();
      if (res != null) {
        let key = Object.keys(res)
        for (let i = 0; i < key.length; i++) {
          this.afDB.database.ref('messages').child(this.userId).child(this.authService.getUser()?.uid).child(key[i]).update(updateRead)
        }
      }
    }).then(() => {
      this.afDB.database.ref('messages').child(this.authService.getUser()?.uid).child(this.userId).orderByChild('userId').equalTo(this.userId).once('value', snapshot => {
        var res = snapshot.val();
        if (res != null) {
          let key = Object.keys(res)
          for (let i = 0; i < key.length; i++) {
            this.afDB.database.ref('messages').child(this.authService.getUser()?.uid).child(this.userId).child(key[i]).update(updateRead)
          }
        }
      })
    })
  ...
  sendNewMessage(message) {
    if (this.userId) {
      var promise = new Promise((resolve, reject) => {
        ...
        // sedn the message
        this.afDB.database.ref('/messages').child(this.authService.getUser()?.uid).child(this.userId).push(messages).then((snap) => {
          var keys = snap.key;
          snap.update({
            key: keys
          })
          this.afDB.database.ref('/messages/').child(this.userId).child(this.authService.getUser()?.uid).push(messages).then((snap) => {
            snap.update({
              key: keys
            })
            //clear the message
            ...
            this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(this.userId).once('value', snapshot => {
              ...
              if (res != null) {
                ...
                this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).child(store[0]).remove().then(() => {
                  //send the conversation to the user base on the ID
                  this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).push(conversation).then(() => {
  ...
  uploadPhotoMessage(sourceType) {
    return new Promise((resolve, reject) => {
      ...
      this.camera.getPicture(this.chatPhotoOption).then((imageData) => {
        ...
        const ref = this.afstorage.ref('/Messaging/' + this.authService.getUser()?.uid + this.generateFilename())
  ...
  deleteForMe(key) {
    this.loading.show();
    this.afDB.database.ref('messages').child(this.authService.getUser()?.uid).child(this.userId).orderByChild('key').equalTo(key).once('value', snapKey => {
      var res = snapKey.val();
      let store = Object.keys(res)
      this.loading.hide();
      this.afDB.database.ref('messages').child(this.authService.getUser()?.uid).child(this.userId).child(store[0]).remove()
    })
  }
  ...
  deleteForEveryOne(key) {
    ...
    this.afDB.database.ref('messages').child(this.authService.getUser()?.uid).child(this.userId).orderByChild('key').equalTo(key).once('value', snapKey => {
      ...
      this.afDB.database.ref('messages').child(this.userId).child(this.authService.getUser()?.uid).orderByChild('key').equalTo(key).once('value', snapKeys => {
        var ress = snapKeys.val();
        let store = Object.keys(ress)
        this.afDB.database.ref('messages').child(this.userId).child(this.authService.getUser()?.uid).child(store[0]).remove();
        this.loading.hide()
      })
    }).then(() => {
