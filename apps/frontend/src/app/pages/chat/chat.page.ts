...
export class ChatPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.conversation(this.authService.getUser()?.uid).valueChanges().subscribe((lengths) => {
  ...
  async deleteConversation(item) {
    await this.afDB.database.ref('conversations').child(this.authService.getUser()?.uid).orderByChild('userId').equalTo(item.userId).once('value', snap => {
      var res = snap.val()
      if (res != null) {
